package com.example.myapplication.constants;

/**
 * Created by Michal on 13/01/14.
 */
public class GcmConstants {
    public static final String SENDER_ID = "505647745249";
    public static final int NOTIFICATION_REFRESH = 1;
    public static final int NOTIFICATION_LOGOUT = 2;
    public static final int NOTIFICATION_JOURNEY_REQUEST_RECEIVED = 3;
    public static final int NOTIFICATION_JOURNEY_REQUEST_ACCEPTED = 4;
    public static final int NOTIFICATION_JOURNEY_REQUEST_DENIED = 5;
    public static final int NOTIFICATION_FRIEND_REQUEST = 6;
    public static final int NOTIFICATION_FRIEND_REQUEST_ACCEPTED = 7;
    public static final int NOTIFICATION_FRIEND_REQUEST_DENIED = 8;
    public static final int NOTIFICATION_FRIEND_OFFERED_NEW_JOURNEY = 9;
    public static final int NOTIFICATION_INSTANT_MESSENGER = 10;
    public static final int NOTIFICATION_JOURNEY_REQUEST_SENT = 11;
    public static final String BROADCAST_ACTION_REFRESH = "BROADCAST_ACTION_REFRESH";
    public static final String BROADCAST_INSTANT_MESSENGER = "BROADCAST_INSTANT_MESSENGER";
    public static final String BROADCAST_ACTION_LOGOUT = "BROADCAST_ACTION_LOGOUT";
}
